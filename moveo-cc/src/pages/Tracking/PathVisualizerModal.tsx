...
const PathVisualizerModal: React.FC<PathVisualizerModalProps> = ({
  ...
  const handleSelectTrip = async () => {
    ...
    try {
      const { apiService } = await import('@/services/apiService');
      const pathData = await apiService.getTripPath(selectedTrip.id);
  ...
  const handleSelectTimeRange = async () => {
    if (!vehicleId || !startDate || !endDate) return;

    try {
      const { apiService } = await import('@/services/apiService');
      const pathData = await apiService.getTimeRangePath(
        vehicleId,
        ...
        {activeTab === 'timerange' && (
          <>
            ...
            <InputGroup>
              ...
              />
              <Button onClick={handleSelectTimeRange} style={{ marginTop: '8px' }}>
                Visualize Path
              </Button>
            </InputGroup>

            {loading && <EmptyState>Loading path data...</EmptyState>}
            
            {!loading && trips.length === 0 && startDate && endDate && (
              <EmptyState>No telemetry data found in this time range</EmptyState>
            )}
          </>
        )}
      </ModalContent>
